...
import jmetal.core.*;
import jmetal.metaheuristics.ccso.Utils;
 ...
 */
public class SMPSO2 extends Algorithm {
	...
	public SMPSO2(Problem problem, Vector<Double> variables,
	 ...
	 *            Problem to solve
	 */
	public SMPSO2(Problem problem, String trueParetoFront)
		...
		for (int i = 0; i < swarmSize_; i++) {
			...
			for (int var = 0; var < particle.getNumberOfDecisionVariables(); var++) {
				...
				speed_[i][var] = velocityConstriction(
						...
						deltaMin_, // [var],
						var, i);
				//speed_[i][var] = speed_[i][var] + PseudoRandom.randDouble()*(speed_[i][var] - oSpeed);
			}
		}
	} // computeSpeed
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (iteration_ < maxIterations_) {
			...
			if(iteration_ < 0.00*maxIterations_) {
				for (int i = 0; i < particles_.size(); i++) {
				...
				computeFitness(particles_);
				getRivals();
				for(int i=0;i<swarmSize_;i++) {
					Solution particle = particles_.get(i);
					SolutionSet globalWinners = new SolutionSet();
					SolutionSet globalLosers = new SolutionSet();
					double normDistance = getPBIDistance(particle, particle);
					for(int j=0;j<T_;j++) {
						Solution sol2 = particles_.get(rivals_[i][j]);
						double compareDistance = getPBIDistance(sol2, particle);
						if(compareDistance < normDistance) {
							globalWinners.add(sol2);
						}else {
							globalLosers.add(sol2);
						}
					}
					if(globalWinners.size() >= 1) {
					    int rdInt = PseudoRandom.randInt(0, globalWinners.size()-1);
					    Solution pp = new Solution(globalWinners.get(rdInt));
						best_[i] = pp;
				    }else {
				    	int rdInt = PseudoRandom.randInt(0, leaders_.size()-1);		
				    	Solution pp = new Solution(leaders_.get(rdInt));
						best_[i] = pp;
					}
				}
			}
	...
	public void computeFitness(SolutionSet solutionSet) {
		for(int i=0;i<solutionSet.size();i++) {
			...
			for(int j=0;j<problem_.getNumberOfObjectives();j++) {
				...
				double value = (sol.getNormalizedObjective(j))/sum;
	...
	public void getRivals() {
		double[] x = new double[swarmSize_];
		int[] idx = new int[swarmSize_];
		for (int i = 0; i < swarmSize_; i++) {
			// calculate the distances based on normalized objective values
			for (int j = 0; j < swarmSize_; j++) {
				if(i == j) { //Rule particle itself out as a rival
					x[j] = Double.MAX_VALUE;
				}else {
					x[j] = Utils.distVector1(particles_.get(i), particles_.get(j));
				}
				idx[j] = j;
			} // for
			// find 'niche' nearest particles as rivals
			Utils.minFastSort(x, idx, swarmSize_, T_);
			System.arraycopy(idx, 0, rivals_[i], 0, T_);
		} // for
	} // getRivals	
	...
	public double getPBIDistance(Solution sol, Solution ref) {
		...
		for (int i = 0; i < sol.numberOfObjectives(); i++) {
			d[1] += (sol.getNormalizedObjective(i) - d[0] * (ref.getNormalizedObjective(i) / refLenSQ))
					* (sol.getNormalizedObjective(i) - d[0]
							* (ref.getNormalizedObjective(i) / refLenSQ));
		}
		d[1] = Math.sqrt(d[1]);
		distance = d[0] + 0.0*d[1];
		return distance;
	}
