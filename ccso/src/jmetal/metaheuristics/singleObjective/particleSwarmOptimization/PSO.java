...
public class PSO extends Algorithm {
	...
	private double constrictionCoefficient(double c1, double c2) {
		...
		if (rho <= 4) {
			...
			return 2 / Math.abs((2 - rho - Math.sqrt(Math.pow(rho, 2.0) - 4.0
					* rho)));
		}
	...
	private void computeSpeed(int iter, int miter) throws JMException,
		...
		for (int i = 0; i < particlesSize_; i++) {
			...
			for (int var = 0; var < particle.size(); var++) {
				...
				speed_[i][var] = inertiaWeight(iter, miter, wmax, wmin)
						* speed_[i][var] + C1 * r1
						* (bestParticle.getValue(var) - particle.getValue(var))
						+ C2 * r2
						* (bestGlobal.getValue(var) - particle.getValue(var));
			}
		}
	} // computeSpeed
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < particlesSize_; i++) {
			...
			if ((globalBest_ == null)
					|| (particle.getObjective(0) < globalBest_.getObjective(0)))
		...
		while (iteration_ < maxIterations_) {
			int bestIndividual = (Integer) findBestSolution_
					.execute(particles_);
			try {
				// Compute the speed_
				computeSpeed(iteration_, maxIterations_);
			} catch (IOException ex) {
				Logger.getLogger(PSO.class.getName()).log(Level.SEVERE, null,
						ex);
			}
			...
			for (int i = 0; i < particles_.size(); i++) {
				...
				// remember
				if ((particles_.get(i).getObjective(0) < localBest_[i]
						.getObjective(0))) {
					Solution particle = new Solution(particles_.get(i));
					localBest_[i] = particle;
				} // if
				if ((particles_.get(i).getObjective(0) < globalBest_
						.getObjective(0))) {
					Solution particle = new Solution(particles_.get(i));
		...
		resultPopulation.add(particles_.get((Integer) findBestSolution_
				.execute(particles_)));
