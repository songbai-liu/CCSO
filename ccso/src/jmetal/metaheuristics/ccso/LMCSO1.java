...
public class LMCSO1 extends Algorithm {
	/**
	...
	public void initParams() {
		...
		groupW_ = new SolutionSet();
		groupL_ = new SolutionSet();
		theta_ = 1.0;
    ...
    **/
    ...
    private double calcAdditiveEpsilonIndicatorTranslatedObjective(Solution sa, Solution sb, int nObj) { 
   		...
   		for(int i = 0; i < nObj; i ++){ 
   			double dif = sa.getNormalizedObjective(i) - sb.getNormalizedObjective(i);
   			if(dif > valEpsilonAB){
   				valEpsilonAB = dif;
   			} 
   		}  
	...
	private void computeSpeed() throws JMException,IOException {
		...
		while(groupW_.size() > 1){
			...
			if(rd <= 0.8){
				value1 = winner1.getDistanceToIdealPoint();
				value2 = winner2.getDistanceToIdealPoint();
			}else{
