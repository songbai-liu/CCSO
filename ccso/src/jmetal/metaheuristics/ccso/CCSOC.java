...
public class CCSOC extends Algorithm {
	/**
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (iteration_ < maxIterations_) {
			...
			theta_ = PseudoRandom.randDouble() + (4.0*iteration_)/maxIterations_;
	...
	private void computeSpeed() throws JMException,IOException {
		...
		for(int i=0;i<groupL_.size();i++) {
			...
			teacher1 = new XReal(winner1);
			teacher2 = new XReal(winner2);
			...
			for(int var=0;var<dim;var++) {
				...
				deltaSpeed += r2*(teacher1.getValue(var)-particle.getValue(var)); 
