...
public class CCSOP extends Algorithm {
	/**
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (iteration_ < maxIterations_) {
			if(iteration_ % 100 == 0){
				IGD[t] = indicators.getIGD1(particles_);
				t++;
			}
			...
			if(remain > 0) {
				frontw.sort(new CrowdingDistanceComparator());
				for(int k = 0; k < remain; k++) {
					groupW_.add(frontw.get(k));
				}
			...
			theta_ = PseudoRandom.randInt(0, 1) + (5.0*iteration_)/maxIterations_;
			...
			groupW_.clear();
			groupL_.clear();
			NondominatedRanking rankings = new NondominatedRanking(union_);
			remain = swarmSize_;
			index = 0;
			SolutionSet front0 = rankings.getSubfront(index);
			estimateIdealPoint(front0);
			translateObjective(union_);
			...
			if(remain > 0) {
				...
				front0.sort(new CrowdingDistanceComparator());
				for (int k = 0; k < remain; k++) {
					particles_.add(front0.get(k));
				}
			}
			for(int i=0;i<swarmSize_;i++){
				Solution particle = particles_.get(i);
		...
		printGD("GCSOP_"+problem_.getNumberOfObjectives()+"Obj_"+problem_.getName()+ "_" + problem_.getNumberOfVariables() + "D_IGD"+".txt", IGD);
	...
	private void computeSpeed() throws JMException,IOException {
		...
		for(int i=0;i<groupL_.size();i++) {
			...
			//leader = new XReal(winner2);
			 ...
			 fit1 = getPBIDistance(loser,loser); for(int j=0;j<size;j++) { if(j != i){
			 Solution sol = groupW_.get(j); fit2 = getPBIDistance(sol, loser); if(fit2 <
			 fit1) { rivals.add(sol); } } } int r = 0; if(rivals.size() > 0) { r =
			 PseudoRandom.randInt(0, rivals.size()-1); winner1 = rivals.get(r); leader =
			 new XReal(winner1); }else { r = PseudoRandom.randInt(0, size-1); winner1 =
			 groupW_.get(r); leader = new XReal(winner1); }
