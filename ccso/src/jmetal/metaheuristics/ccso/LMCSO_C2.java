...
public class LMCSO_C2 extends Algorithm {
	/**
	...
	public LMCSO_C2(Problem problem) {
		super(problem);
	}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (iteration_ < maxIterations_) {
			...
			theta_ = 1.0 + (5.0*iteration_)/maxIterations_;
	...
	private void computeSpeed() throws JMException,IOException {
		...
		for(int i=0;i<size;i++) {
			...
			index2 = winner.getindex();
			fit1 = getPBIDistance(winner, winner); 
			XReal particle = new XReal(winner);
			Solution teacher;
			for(int j=0;j<size;j++) {
				if(j != i){
					sol =  groupW_.get(j);
					fit2 = getPBIDistance(sol, winner);
					if(fit2 < fit1) {
						rivals.add(sol);
					}
				}
			}
			if(rivals.size() > 0.05*size) {
				winner.setLearningType(1);
				int rd = PseudoRandom.randInt(0, rivals.size()-1);
				teacher = rivals.get(rd);
				XReal leader = new XReal(teacher);
				double r1 = PseudoRandom.randDouble();
				double r2 = PseudoRandom.randDouble();
				for(int var=0;var<dim;var++) {
					double deltaSpeed = r1*speed_[index2][var];
					deltaSpeed += 1.0*r2*(leader.getValue(var)-particle.getValue(var)); 
					if(deltaSpeed > deltaMax_[var]) { deltaSpeed = deltaMax_[var]; }
					if(deltaSpeed < deltaMin_[var]) { deltaSpeed = deltaMin_[var]; }
					speed_[index2][var] = deltaSpeed;
				}
			}else {
