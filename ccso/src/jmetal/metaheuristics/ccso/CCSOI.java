...
public class CCSOI extends Algorithm {
	/**
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		SolutionSet mid = new SolutionSet();
		// -> Step4 Iterations
		while (iteration_ < maxIterations_) {
			...
			int avg = swarmSize_/5;
			for(int k = 0; k < 2*avg; k++) {
				groupW_.add(particles_.get(k));
			}
			for(int k=swarmSize_-1;k>=3*avg;k--) {
				groupL_.add(particles_.get(k));
			}
			for(int k=2*avg;k<3*avg;k++){
				mid.add(particles_.get(k));
			}
			//distance.crowdingDistanceAssignment(particles_,problem_.getNumberOfObjectives());
			SDEDistanceAssign(particles_);
			mid.sort(new CrowdingDistanceComparator());
			if(PseudoRandom.randDouble() < 0.5){
				mid.sort(new CrowdingDistanceComparator());
			}else{
				mid.sort(new FitnessComparator());
			}
			for(int k=0;k<mid.size()/2;k++){
				groupW_.add(mid.get(k));
			}
			for(int k=mid.size()/2;k<mid.size();k++){
				groupL_.add(mid.get(k));
			}
			theta_ = PseudoRandom.randDouble() + (5.0*iteration_)/maxIterations_;
			...
			if (setFront.size() > 1) {
			  ...
			  // compute offsets for reference point in normalized space
			     ...
			     for (int i = 0; i < maximumValues.length; i++) {
			        offsets[i] = 100.0 / (maximumValues[i] - minimumValues[i]);
			     }
