...
public class LZ09 {
	...
	void alphaFunction(double alpha[], Vector<Double> x, int dim, int type) {
		if (dim == 2) {
			...
			if (type == 24) {
				alpha[0] = x.elementAt(0);
				alpha[1] = 1 - x.elementAt(0) - 0.05
						* Math.sin(4 * Math.PI * x.elementAt(0));
			}
		} else {
			if (type == 31) {
				alpha[0] = Math.cos(x.elementAt(0) * Math.PI / 2)
						* Math.cos(x.elementAt(1) * Math.PI / 2);
				alpha[1] = Math.cos(x.elementAt(0) * Math.PI / 2)
			...
			if (type == 33) {
				...
				alpha[2] = 3
						- (Math.sin(3 * Math.PI * x.elementAt(0)) + Math.sin(3
								* Math.PI * x.elementAt(1))) - 2
	...
	void objective(Vector<Double> x_var, Vector<Double> y_obj) {
		// 2-objective case
		if (nobj == 2) {
			if (ltype == 21 || ltype == 22 || ltype == 23 || ltype == 24
					|| ltype == 26) {
				double g = 0, h = 0, a, b;
				...
				for (int n = 1; n < nvar; n++) {
					...
					if (n % 2 == 0) {
						a = psfunc2(x_var.elementAt(n), x_var.elementAt(0), n,
								ltype, 1); // linkage
						aa.addElement(a);
					} else {
						b = psfunc2(x_var.elementAt(n), x_var.elementAt(0), n,
								ltype, 2);
						bb.addElement(b);
					}
			...
			if (ltype == 25) {
				...
				for (int n = 1; n < nvar; n++) {
					if (n % 3 == 0) {
						...
						c = psfunc2(x_var.elementAt(n), x_var.elementAt(0), n,
								ltype, 3);
						if (n % 2 == 0)
							aa.addElement(c);
						else
		...
		if (nobj == 3) {
			if (ltype == 31 || ltype == 32) {
				...
				for (int n = 2; n < nvar; n++) {
					a = psfunc3(x_var.elementAt(n), x_var.elementAt(0),
							x_var.elementAt(1), n, ltype);
					if (n % 3 == 0)
						aa.addElement(a);
					else if (n % 3 == 1)
						bb.addElement(a);
